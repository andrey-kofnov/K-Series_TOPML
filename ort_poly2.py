...
class K:
    ...
    def Gram_Smidt_process(self, var_index=1, max_poly_degree=-1):
        ...
        for pol_num, new_poly in enumerate(polys_to_orthogonalise):
            ...
            for it, ort_poly in enumerate(orthogonal_poly):
                ...
                coef1 = self.integral(func_inner_prod, params, infty=self.is_Infty) / self.integral(func_sq_norm,
                                                                                                    params,
                                                                                                    infty=self.is_Infty)
    ...
    def Fourier_expansion(self, function, max_poly_degree=-1):
        ...
        orthogonal_poly, polys_coef_matrix = self.create_orthogonal_poly_collection(
            max_poly_degrees=[max_poly_degree] * len(params.keys()))
